...
|---|---|---|
| 22.1 | **Condition on the signature.** Round 7 threw away 55 of 136 well-formed answers writing a signature nobody asked for. Giving every training document a head made the model emit heads between documents, which the answer splitter did not know about, so 216 of 232 answers failed to parse; the splitter is fixed and the arm is rerunning. Greedy decoding, tried in the same round, took signature failures 40.4% to 32.2% for free. | signature failures below 20% of well-formed answers, on a denominator above 100 ([`locallm/FINDINGS-round7b-2026-09-19.md`](locallm/FINDINGS-round7b-2026-09-19.md)) |
