...
Measured merged (5544a99; tmux strlib-r20, log ~/.local/share/tjob/strlib-r20.log): the 26-task matrix at 16 jobs, the 23 older rows cell for cell as banked, 22 of 26 in all seven (swap_rows in six, framac alone; split_join in three, dafny, verus and lean; count_vowels in two, dafny and rocq); the family on the corpus's 17 names at flake 3 and 24 jobs, 37 seconds: dafny 15, verus 8, spark 5, lean 4, rocq 2, fstar 2, framac 1 of 17, 0 disagreements, 0 against ground truth, 0 twins surviving, 5 no-flip cells (rocq 2, dafny 1, spark 1, verus 1), none in all seven (spark's three abstains are one defect: a family local named `rows` collides with the emitted package's own names); ground truth 471 tasks, 3,249 cells in 776 seconds, 0 REFUTES-TRUE, the one standing framac spec-function cell UNSOUND as in every run since 2026-09-02. No sweep this wave: no lifted DafnyBench task uses a string member (the lifter refuses `string-lib` by name) and every older task lowers byte-identically, so the fifteenth Reading stands.
