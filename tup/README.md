...
| `boot_witness.sh` | Boots the finished disk under QEMU with **nothing but firmware and the disk** (EDK2 for arm64, SeaBIOS for x86_64; no `-kernel`, no `-initrd`, no `-append`) and records the console transcript. The acceptance test is a conjunction, not a sighting: `boot_verdict.sh` has it. |
| `boot_verdict.sh` | The one definition of “tup booted”, sourced by `boot_witness.sh` and by `release.sh`'s ship gate rather than copied into each. BOOTED is banked only if a `tup login:` line is in the transcript, no kernel panic is anywhere in it, the guest is still alive at the moment the verdict is taken, and the settle window really elapsed; any one failing names its own verdict. The file records how that set of clauses was derived from what a witness can observe, and what it does not cover. |
