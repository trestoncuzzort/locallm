#!/bin/bash
# OVERRIDE ch07/06-createfiles. Two edits to the book's own bytes, both named:
...
set -e
PAGE=""
for cand in /tup-build/book/ch07-inner/06-createfiles.sh \
            /tup-build/book/ch07/06-createfiles.sh \
            /home/lfs/book/ch07/06-createfiles.sh; do
  [ -s "$cand" ] && { PAGE="$cand"; break; }
done
[ -n "$PAGE" ] || { echo "override: cannot find the createfiles page"; exit 1; }
