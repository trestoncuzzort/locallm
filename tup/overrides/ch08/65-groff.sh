#!/bin/bash
# OVERRIDE ch08/65-groff: fill the book's <paper_size> placeholder.
#
...
set -e
PAGE_SRC=""
for c in /tup-build/book/ch08/65-groff.sh /home/lfs/book/ch08/65-groff.sh; do
  [ -s "$c" ] && { PAGE_SRC="$c"; break; }
done
[ -n "$PAGE_SRC" ] || { echo "override: cannot find the groff page"; exit 1; }
