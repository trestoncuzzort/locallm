...
tested on a few examples — *proved*, for every possible input, by an outside
...
Multiple `ensures` lines are all promised at once — an invisible "and"
joins them.
...
- `p implies q` — the one that's new to most beginners. Read it as
  ...
  is false, the whole thing is true by default — it promised nothing about
...
**Assignment** — `r := x + 1` — "compute the right side, store it in the
...
`requires len(s) > 0` — "don't call me with an empty list." Inside the
...
write a separate line per element — you don't know how many there are. You
quantify:
...
top*, checking its condition. Before the first lap, after each lap — the
...
factorial any way it likes — a loop, or by calling *itself* recursively (a
...
`ensures r == fact(n)` — good. "ensures r == whatever this task returns" —
...
lessons 1–11 catches `ensures r >= 0` standing alone on `abs` — plenty of
