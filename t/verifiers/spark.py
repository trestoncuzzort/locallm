...
#
# One pattern, unchanged. What changed 2026-09-05 is upstream: the Import
# row is a bare word a task identifier CAN spell, and the scan is IGNORECASE
# while lower_spark.py capitalizes every t name, so a t param named `import`
# emits `Import` and scores the file VACUOUS whatever gnatprove said —
# "accepted for the wrong reason" about a real proof. Measured on the
# sibling adapters 2026-09-05 (dafny `assumed`: "1 verified, 0 errors" at
# exit 0; fstar `admitted` and `magicNumber`: 4 solver-logged unsat at exit
# 0; lean `trustCompiler`: exit_code -1, nothing ran — each a false
# VACUOUS); gnatprove is not installed on the box this edit was made on, so
# this row is a BY-READING change, unexecuted. lower_spark.py now tests
...
# have come from a construct. The multi-token pragma/aspect rows no single
# identifier can match cost the guard nothing: it never refuses on them.
BANNED = re.compile(
    r"pragma\s+Assume\b"
