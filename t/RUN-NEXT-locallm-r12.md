...
| A3 | **`t/AGREEMENT.md` holds 1 task**: a one-task `run_par.py` run (`--tasks t/nested --jobs 7`, no `--table`) overwrote the 35-task matrix in `a3c6f955` (an earlier draft of this row said 43; `t/tasks/` has 35 files and `0bbc05cb`'s table 35 rows); the corpus builder reads it (`loop_locallm.py:204`), so a corpus built today silently loses the committed tasks | NOT restored from `0bbc05cb`: verdict-deciding code changed since (`lower_lean` +549, `lower_framac` +769, the parser in `surface.py`/`check_wf.py`/`expand_helpers.py`, `interp.py`, `verifiers/spark.py`), so all 35 tasks are regraded on the lab (`bash t/grade_lab.sh matrix`, 245 cells) and the reviewed `t/out/AGREEMENT-lab.md` copied over; `run_par.py` refuses to write the committed table from a strict subset of tasks or kernels unless `--allow-subset-table`, and from changed or foreign task files outright, unless `--table` names another file (2026-09-25) | a subset run leaves `AGREEMENT.md` byte-identical (`t/test_run_par_guards.py`, including the a3c6f955 command replayed) | Jest keeps a filtered run from pruning its snapshot file, jest-circus `jestAdapter.ts` `_addSnapshotData` (receipt fe911737db40); AGREEMENT.md has run-wide fields, so it refuses instead of merging |
...
| A8 | **SPARK `-j8` oversubscribes any sweep that is not `grade_lab.sh`** | `run_par.py` hands every pool worker `T_SPARK_JOBS=1` when more than one cell is in flight, unless the parent set it (2026-09-25) | the workers see `T_SPARK_JOBS=1` and the parent does not (`t/test_run_par_guards.py`). The earlier claim here, "a 24-job sweep stays under the core count", was false even at `-j1`: a cell is six concurrent kernel calls, so the bound is 6 x jobs <= cores, about 20 jobs on 120 threads (24 x 6 = 144; at `-j8` it was 1,152 provers and a load of 351) | joblib caps inner threads in each worker's environment unless the parent set them, `_parallel_backends.py` `_prepare_worker_env` (receipt 7703c680c129); BenchExec (b55d4c4555c8) |
