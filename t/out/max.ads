pragma Ada_2022;
with Ada.Numerics.Big_Numbers.Big_Integers;
use  Ada.Numerics.Big_Numbers.Big_Integers;
package T_Max with SPARK_Mode is

   function F (X : Big_Integer; Y : Big_Integer) return Big_Integer
   with
     Post => (F'Result >= X)
       and then (F'Result >= Y)
       and then ((F'Result = X) or else (F'Result = Y));

   function F (X : Big_Integer; Y : Big_Integer) return Big_Integer is
     ((if (X >= Y) then X else Y));

   --  The task at machine inputs with exact arithmetic: gnatprove's RAC
   --  cannot execute a Big_Integer expression, so F above can never carry a
   --  confirmed counterexample (header). Every magnitude here is bounded by
   --  construction, so a counterexample to F_Ce is a counterexample to the
   --  task itself.
   subtype Ce_Num is Long_Long_Long_Integer;
   subtype Ce_Int is Ce_Num range -2**40 .. 2**40;

   function F_Ce (X : Ce_Int; Y : Ce_Int) return Ce_Num
   with
     Post => (F_Ce'Result >= X)
       and then (F_Ce'Result >= Y)
       and then ((F_Ce'Result = X) or else (F_Ce'Result = Y));

   function F_Ce (X : Ce_Int; Y : Ce_Int) return Ce_Num is
     ((if (X >= Y) then X else Y));

end T_Max;
