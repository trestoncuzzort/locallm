...
- 2026-09-17 17:45 ROADMAP.md gains WS-20, what caps the corpus, with the three moves in order: nested-loop lowering for Lean, Rocq and F* (any loop inside a loop abstains in three of the seven, so the corpus cannot hold a real algorithm and today's 96 clean answers are short and loop-shallow because nothing else can pass; the measurement to open it with is how many nested-loop programs the four kernels that do take them already prove), then more problem sources (pool v4 is 737 problems and split-v4 leaves 505 for training, while nl/ holds 4,239 function-shaped problems of which 772 read as inside t's fragment, and APPS and CodeContests are not downloaded here; a test reader per source raises the ceiling to thousands, with the held-out split staying exactly split-v3's 232), then the twins and witnesses as a first-class artifact (one file per task with the real program, the twin, the operator, the witness and the seven verdicts, published and versioned like AGREEMENT.md, since a program paired with a near-miss and the input that separates them is the part no other corpus has).
- 2026-09-17 18:05 the real cause of seed 7's hang, found by looking at the lab workstation's disk: mbpp_478__remove_lowercase chains 26 string `replace` calls, and the Rocq lowering expands each inline, so it wrote a 27.7 GB source for the real program and another for its twin. 52 GB of a shared disk, and the run wedged on it for hours (three times). The files are deleted, the directory is back to 103 MB and the disk went from 63 GB free to 115 GB. run_par.py now records a lowered source over T_MAX_SOURCE_MB (64 MB) as lower-too-big and never runs it, so a lowering that explodes cannot fill a disk or wedge a run again. This is a lowering bug to fix under WS-20: `replace` should not nest inline.
- 2026-09-17 18:20 t/yield.py: what one verified example costs, per answer set and in total, from each reply's own wall_s and from the check events. Measured over everything graded so far: 8,848 generations, 2,695 well formed, 1,896 passing their tests, 1,230 graded, 203 clean over 76 distinct problems. Per accepted example: 43.6 generations, 540 s of generation and 3,344 s of proof (cells run in parallel, so clock time is far lower than the sum). These are the two numbers that say whether the loop scales: generations per accepted example, which a better generator lowers, and proof seconds per accepted example, which the lowerings and the kernels decide.
- 2026-09-17 18:26 start `more-problems`: `for S in 1 2 3 4 5 6 7 8; do T=qwen2.5-coder-14b-he-s$S; D=t/out/spec-experiment/$T; [ -d $D/grade-in ] && continue; TEMP=0.7; [ $S = 1 ] && TEMP=0; python3 t/spec_experiment.py generate --model qwen2.5-coder:14b --tag $T --pool v4 --min-id 100000 --prompt v3 --seed $S --temperature $TEMP --num-ctx 8192 --num-predict 3072 --timeout 1800 --jobs 4 && python3 t/spec_experiment.py extract --model $T --pool v4 && python3 t/spec_experiment.py tests --model $T --pool v4 && python3 t/pool_pick.py $D || exit 1; done && ollama pull deepseek-coder-v2:16b && for S in 1 2; do T=deepseek-coder-v2-16b-v4-s$S; D=t/out/spec-experiment/$T; [ -d $D/grade-in ] && continue; TEMP=0.7; [ $S = 1 ] && TEMP=0; python3 t/spec_experiment.py generate --model deepseek-coder-v2:16b --tag $T --pool v4 --prompt v3 --seed $S --temperature $TEMP --num-ctx 8192 --num-predict 3072 --timeout 1800 --jobs 4 && python3 t/spec_experiment.py extract --model $T --pool v4 && python3 t/spec_experiment.py tests --model $T --pool v4 && python3 t/pool_pick.py $D || exit 1; done` (log `logs/more-problems.log`)
- 2026-09-17 18:26 t lab allows one step on this desktop and one on the lab workstation at the same time, and refuses a second on either; the desktop file says the same.
- 2026-09-17 18:37 start `grade-repair`: `bash t/grade_lab.sh tags qwen2.5-coder-14b-v3-s1-fix1 qwen2.5-coder-14b-v3-s2-fix1 qwen2.5-coder-14b-v3-s3-fix1 qwen2.5-coder-14b-v3-s4-fix1 qwen2.5-coder-14b-v3-s5-fix1 qwen2.5-coder-14b-v3-s6-fix1 qwen2.5-coder-14b-v3-s7-fix1 qwen2.5-coder-14b-v3-s8-fix1` (log `logs/grade-repair.log`)
- 2026-09-17 18:47 speed, measured before changing anything. Cutting the prover timeout was rejected: at a 45 s budget 395 cells that currently read verified / refuted would be lost (337 of them SPARK, which is slow because it is proving, not because it is giving up), against 20 h of prover time saved. The lab workstation is back at its default 16 jobs (the 4-job setting was only for the hang hunt, and the hang was the 27 GB lowering). The repair round on the new answer sets and its grading are removed from the steps: 51 repaired answers graded, 0 clean, 8 worse against 4 better, so a second copy of that result costs about an hour of the card and hours of proving and answers nothing new. Generation stays at 4 answers in flight: more would push the KV cache off the 16 GB card and into memory, which is slower, not faster.
- 2026-09-17 18:59 start `grade-repair`: `bash t/grade_lab.sh tags qwen2.5-coder-14b-v3-s1-fix1 qwen2.5-coder-14b-v3-s2-fix1 qwen2.5-coder-14b-v3-s3-fix1 qwen2.5-coder-14b-v3-s4-fix1 qwen2.5-coder-14b-v3-s5-fix1 qwen2.5-coder-14b-v3-s6-fix1 qwen2.5-coder-14b-v3-s7-fix1 qwen2.5-coder-14b-v3-s8-fix1` (log `logs/grade-repair.log`)
- 2026-09-17 18:59 end `grade-repair`: exit 0 after 0 min
- 2026-09-17 19:00 the repair round is measured in full: 110 repaired answers graded, 2 clean (both in seed 6), 1.8 percent, against about 21 percent clean for fresh samples from the same model. Self-repair from verdicts is an order of magnitude worse than resampling. The pool stands at 213 clean over 78 distinct problems.
- 2026-09-17 19:00 start `grade-growth`: `bash t/grade_lab.sh tags qwen2.5-coder-14b-he-s1 qwen2.5-coder-14b-he-s2 qwen2.5-coder-14b-he-s3 qwen2.5-coder-14b-he-s4 qwen2.5-coder-14b-he-s5 qwen2.5-coder-14b-he-s6 qwen2.5-coder-14b-he-s7 qwen2.5-coder-14b-he-s8 deepseek-coder-v2-16b-v4-s1 deepseek-coder-v2-16b-v4-s2` (log `logs/grade-growth.log`)
- 2026-09-17 19:01 the two repair steps are removed from the list now that their result is measured (2 clean of 110): they demanded a repair set for all eight seeds, seed 7 was graded too late to have one, so they stayed visible as unfinished forever while adding nothing. The growth grading no longer demands answer sets that were never written either. What is left: the second generator, its grading, the pool, the student, locallm, the held-out grading and the score.
