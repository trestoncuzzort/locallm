# tup and t: the engineering log

This file is the dated record behind `ROADMAP.md` at the repository root: every wave, sweep and measurement, with its reasoning, appended in place as it happened. New readers should start with `ROADMAP.md`; the section numbers (WS-7 to WS-19, 13.1 to 17.3) are the same in both files.
...
- Adapter audit, seven hostile builders, one per kernel, each trying to make
...
| **agent** | raw twin | `xbps` or `apk` | The layer needs the small userland a package manager actually reaches for (git, curl, ripgrep, jq) and needs it to keep moving. Both are small, daemonless, single-binary, and FHS-shaped. |
...
four implementers and an integrator, fixed in two passes, reviewed
...
a second independent fetch of nearly every number; PDF in ~/tup/tup-reports,
every source saved under ~/tup/tup-reports/papers). On 2026-09-10 every move
...
with the rest of tonight's parallel work (other ROADMAP items' builders were
...
the merge, and the quiet re-run of the suite is the wave's gate. Quiet re-run (16:54Z, no other kernel work on the box): 261 of 462 PASS, 201 FAIL, one fewer than the loaded run, so the FAILs are kernel incompleteness against the probes' own expectations, not contention; by column dafny 24, verus 25, spark 24, framac 36, lean 33, rocq 31, fstar 28. The framac cell on `fz_p_divreq0` read VERIFIED both times: Frama-C accepts a task whose `requires` is undefined at every input (a division by zero inside the precondition), which WP reads as a false precondition and proves everything under; the verifier's vacuity smoke (verifiers/framac.py) does not fire on it. That is the one soundness-direction cell in the suite and the next framac verifier fix by name. Fixed the same day (15fdabd): interp tells a requires undefined at every input from one false everywhere, the harness refuses the first by name (`vacuous-requires-undefined`) in every column before any kernel runs, and lower_framac emits each requires clause's definedness as a companion requires, so WP's own smoke reads the probe vacuous; the probe's expectation is `vacuous`, framac now PASSes it and the other six columns read unproved (they prove nothing under the undefined precondition and cannot name it), so the suite reads 262 of 462 PASS on the re-run (17:35Z) with the 34-row matrix unchanged. With the real's own refutation witness routed into the suite's driver the same way (f2873b9 and the commit after ebf220a), 341 of 462 PASS: the probes expecting a refuted real now get their certificate. The 121 left: five probes fail in every column (badrec, badrec2, badvariant, whose expectation is a rejection of a bad measure the outcome vocabulary spells otherwise, and biglen, seqlen, whose witness is a length the bounded domain cannot enumerate); the undefined-access family (at_oob, at_neg, at_zero, at_body, attotal) and the vacuity probes (divreq0, vac_post, vac_unsat, vac_range) in most columns; and per kernel dafny 1, verus 2, spark 4, fstar 4, lean 8, rocq 8, framac 15 cells of their own, the string-library edge cases (empty separator, tab, a non-letter through lower) and framac's nested and pair returns among them. Pass 1 (293c796, one harness-level builder and one per kernel, each checked): 406 of 462 PASS, 56 FAIL. The suite gained an expectation class, `rejected` (refuted, unproved, malformed or lower-error, never verified, timeout or vacuous), for the five probes whose point is that a kernel must not verify (a non-well-founded measure, a length no machine type bounds) and for the undefined requires; vac_post expects the twin rule's `decorative`; lean, rocq and fstar gained the vacuity smoke the other four verifiers had; per kernel dafny, verus, lean and fstar closed every cell of their own, spark three of four, rocq four of ten, framac two of fifteen. The 56 left, by name: the undefined-access family (at_oob, at_neg, at_zero, attotal) in the six columns other than framac, an ensures-level definedness violation no lowering's certificate path replays (a forced witness regressed framac and was reverted); framac's 20, its seq-valued expressions, nested and pair returns and string members in executable position, the CAPACITY gap one construct up; rocq's five string facts and the seq pair component (its prelude is emitted whole for every task, so a fact added for one probe costs every proof); spark's content-free contract reading of modsign_true, badrec2's timeout and at_body; vac_post in spark, lean and fstar where the twin does not also verify. The 34-row matrix moved one cell, named before the merge: split_join's fstar real from unproved to timeout as its prelude grew. Pass 2 (7006cf7, the harness's ensures-level undefined witness and its certificate in six lowerings, rocq's four and dafny's and lean's cells, spark's at_body): 425 of 462 PASS, 37 FAIL. Two things the merged gate showed that the builders' own worktree measurements did not: framac, which already refuted the four undefined-access probes through its definedness companions and was given no builder for the new witness, now times out on them (the same regression pass 1 had measured and reverted, this time from the harness side), and verus's four did not reach refuted through the merged pipeline; both fixed in the commit after (verus builds the certificate from the offending node's own definedness predicate; framac keeps its value certificate, the witness now carrying the body's value): 433 of 462 PASS, 29 FAIL, every one of them named below. Still open by name (the 29): framac's 20 architectural cells, rocq's five string facts and the seq pair component (the prelude split was not attempted), spark's content-free contract reading, badrec2's timeout and vac_post. split_join's fstar real reads unproved or timeout run to run, at the budget's edge. Pass 3, rocq (the commit after 5843aec): t/lower_rocq.py's prelude is now a core plus a string block emitted only when the task or its body uses a string member (lower_fstar.py's _uses_strlib gate, ported), so the 29 committed tasks with no string member each lose 699 lines of prelude and the three string tasks keep it; four base-case facts (t_list_zero, t_count_list_empty, t_find_list_empty, split_ws_list_empty) live inside the block and close fz_p_str_countempty, fz_p_str_findempty and fz_p_str_splitempty. The merged gate (21:28Z, quiet box): 436 of 462 PASS, 26 FAIL, the 34-row matrix unchanged cell for cell. The 26 by name: framac's 20 as above; spark's modsign_true, vac_post and badrec2; rocq's fz_p_str_tab and fz_p_str_lowernonletter (both need a pointwise map or index lemma the prelude has no version of, coqc's message unchanged: Tactic failure: unsolved t verification condition) and fz_p_pair_seq (a structural refusal of a seq pair component in _check_pair_types, raised before any proof, not a prelude gap). Pass 4 (the commit after 2b3f6d9, three builders each independently checked): 444 of 462 PASS, 18 FAIL, the 34-row matrix unchanged cell for cell. Spark's column is clean: its havoc oracle (a postcondition that still proves when the body is replaced by an arbitrary result) is recorded as evidence and no longer names the outcome, matching what the other six smokes measure (an unsatisfiable requires; the harness's decorative_kind owns the content-free postcondition), so modsign_true and vac_post read verified; badrec2 reads unproved because the counterexample search budget is now its own constant (gnatprove's RAC was executing the non-terminating spec function to confirm a countermodel) and a run whose variant check gnatprove already gave up on but whose whole-file reading was TIMEOUT is re-run once at a larger deterministic step budget and taken only if decisive, and badvariant moved with it. Framac closed five: divreq0 because the harness's vacuous-requires-undefined refusal now applies to the real in the suite's driver as it did to the twin, so all seven columns read lower-error; at_body because the undefined-witness certificate now descends into if, deciding each branch by interp as the value certificate does; lit_empty, concat_len and lit_index because a seq literal indexed by at renders as a C compound literal and the length of a concat or slice expression as a closed form over the operands' own lengths, never a fresh buffer. One mechanism the framac builder proposed was not merged: reading a TIMEOUT as UNPROVED when Qed alone reads Unknown on every spinning goal. WP runs Qed before alt-ergo on every goal, so a goal alt-ergo spun on is one Qed could not close and its Unknown adds nothing; measured at the merge on an axiom-free file, `lemma l: 0 < 0;` reads Stepout under alt-ergo 2.4.3 and Timeout under Z3 4.8.12 through a private why3 config, so no prover status this column can reach names a false fact. The 18 by name: framac's badrec, badrec2 (vacuous), badvariant, biglen, seqlen (timeout), whose honest route is the one REFUTED already takes here, a measure witness the harness computes replayed as a ground certificate; framac's ten seq cells still abstaining, each with its own measured message in lower_framac.py's note (nest_empty and str_splitempty need a nested-seq return, str_countempty and str_findempty a recursive ACSL definition of count and find, str_tab nested locals, str_lowernonletter an executable lower, nest_cell and nest_lit a conditionally evaluated at, nest_eq an extensional equality loop, pair_seq a struct with a seq component); rocq's str_tab and str_lowernonletter (a pointwise map lemma) and pair_seq (structural). Pass 5 (wave G, the commit after 72e6b94): 450 of 462 PASS, 12 FAIL, the 34-row matrix unchanged cell for cell across six lowerings changed in the same wave. Rocq's column is clean: the string block gained the pointwise lemmas (t_list_zero's siblings for lower and split_ws through is_ws) and pair_comp_ty, pair_ty, rty and param_binders accept a seq pair component, so str_tab, str_lowernonletter and pair_seq read verified (the twin of the last two reads unproved, a certificate the rocq builder does not ground for a forall-over-range or a pair-projection witness, named in its note; the cells grade on the real). Framac closed three: badvariant through the measure witness (harness.real_witness now runs interp with measure checks on and reports the site, the concrete arguments and both measures when a decreases clause fails to decrease or goes negative; lower_framac replays it as a ground lemma Qed proves; the 34 committed witnesses are unchanged), and str_countempty and str_findempty through recursive ACSL definitions of count and find in executable position with an inductive C helper bridging the closed form. The wave's one regression was caught by this gate and fixed before the commit: the twin ladder's new wrong-operator rung swapped seq concatenation's + for -, interp raised on the twin, and lit_empty read a poisoned cell in all seven; the rung now swaps + only when an operand is provably int, and an ill-typed candidate is skipped, never a cell. The 12 by name, all framac: badrec and badrec2 (the measure witness is computed and its certificate emitted, but WP takes the recursive logic definition as an axiom, the axiom is inconsistent, and the verifier's vacuity smoke reads the file VACUOUS before the certificate goal is reached; the next framac item withholds the definition of a spec function whose measure the harness refutes and lets the certificate stand alone), biglen and seqlen (a false bound on an unbounded length no prover here names, timeout, open by design until a symbolic witness exists), and the eight seq cells named in pass 4 minus count and find. Pass 6 (wave H, the commit after 116f076): 454 of 462 PASS, 8 FAIL; the matrix moved one cell, named and banked: split_join's rocq twin from unproved to refuted, the seq-return certificate the rocq builder added for 16.2 grounding it. Framac closed four: badrec and badrec2 read refuted because a spec function whose measure the harness refutes is emitted as a declared logic function without its definition (the inconsistent axiom is gone, the ground certificate stands alone, the other 33 tasks lower byte-identically); nest_cell and nest_lit because code_ats threads the path condition through and, or, implies and ite and at_asserts renders a guarded definedness obligation as an implication. The 8 left, all framac: biglen and seqlen (open by design, see pass 5), nest_eq (an extensional equality loop), nest_empty and str_splitempty (a nested seq return), str_tab (nested locals), str_lowernonletter (an executable lower), pair_seq (a pair with a seq component), each with its measured message in lower_framac.py's notes. Pass 7 (wave I, the commit after e4b16a0): 454 of 462, the same eight, matrix unchanged; nest_eq's message moved from a crash in pred() to the honest refusal (extensional equality in executable position needs a loop rendering path), the other five re-measured byte-identical, each a design the rendering sites alone cannot supply: a second capacity dimension for nested returns and locals, an executable lower with a length bound, a pair with a seq component through the buffer encoding. Pass 8 (wave J, the commit after c9e0030): 454 of 462, the same eight, matrix unchanged; the framac builder spent the wave on 16.2's rows (appendArrayToSeq closed, a capacity-mode assignment bug fixed under it) and did not reach the six seq cells.
...
**15.5 built 2026-09-11 (the same commit); one clause reachable only after the next commit, one clause for a person.** The verdicts view is a TreeDataProvider fed by the server's `t/verdicts` notification: one node per kernel, "real / twin" with the operator, the witness rendered as the input or exit state the server sends, "(provisional)" when the runs disagreed, "absent" for a kernel whose entry says the binary is missing, the kernel's message for a refuted real, and a status bar count of agreeing kernels. Measured through the same stdio session the extension opens: abs.t gives dafny verified / refuted with the collapse-if witness, the same cell as AGREEMENT.md; a kernel forced absent reads absent, never a verdict. The clause "a real task REFUTED with the kernel's message" was found unreachable by the builder (the library lowered the real without a refutation certificate, and REFUTED is certificate-only in dafny and fstar by design) and closed the same day (f2873b9): harness.real_witness finds, with the interpreter alone, an input on which the real body violates its own ensures, and tlib.verify and run_par.lower_and_dispatch lower the real with it as the certificate witness, the route the ground-truth family already took; t/editors/examples/wrong_abs.t, wrong_sum_upto.t and wrong_last.t read refuted in all seven columns with the kernel's message, every committed task has no such witness, and the 34-row matrix is unchanged (the gate run after the merge). The walk-through step pastes the measured verdict. Open: the person-run walk-through.
...
**16.2, unbounded-quantifier row, 2026-09-12.** `bound_quantifier` (t/lift_classify.py) read only one shape per unguarded/multi-binder combination; dafny's own `rprint` (what the lifter actually feeds it) produces at least four more it never matched, all still an index bounded by the array/seq it indexes -- a leftover conjunct beside the range chain that used to make the single-binder branch bail outright, a range distributed across body's own top-level `&&`/`||` (`range ==> (A && B)` prints as `(range ==> A) && (range ==> B)`, and the `exists` analogue with `||`), an unguarded MULTI-binder quantifier (the N-binder branch had no unguarded path at all before today), and two binders ordered against the same end in one chain (`0 <= i < j < hi`, read as `i` in `[lo,hi)` and, nested, `j` in `[i+1,hi)`). LIFTER-DECISIONS.md row 31 (t/lift_classify.py, t/test_lift_rules.py, six new unit tests). Measured on the 30 in-fragment dafny-synthesis programs COVERAGE-mbpp-dfy-lifter.md names for this row (`t/lifter.py --list <164> --skip-check`, restricted to dafny-synthesis): 22 of 30 now lift (classify+rewrite; the other 8 split 2 genuinely unbounded and 6 a distinct `| _t#0 == i + 1` filter-binder shape, out of this row's scope); every one of the 177 files the lifter already emitted before this change re-lifts byte-identical (diffed, before vs. after). Of the 22, 2 (task 126 `SumOfCommonDivisors`, task 603 `LucidNumbers`) also pass `lift_check` (dafny alone); the other 20 fail `lift-check-failed`, a separate, pre-existing checker gap (seq-typed locals, capacity bounds on a seq return, char-membership quantifiers) this row does not touch. Graded at flake 3 in all seven kernels (t/grade.py --tasks <22> --flake 3 --jobs 8): 0 of 22 reach `verified/refuted` agreement in all seven -- every task hits at least one kernel-side ABSTAIN, timeout, or `invariant-drop` disagreement (lean's uninitialized-seq-local gap, framac's seq-return-capacity gap, spark timeouts at `--steps 20000`, and on two tasks verus proves the real AND its twin, an existing verus soundness gap this row surfaces but does not cause). So: this row moves the LIFTER side (30 -> 8 in-fragment unbounded-quantifier refusals), the 82-cell count is unmoved by it alone, and the next hurdle is named by the table above it, unchanged: kernel-side blockers of what the lifter now emits.

2026-09-12: the `lift-check-failed` row worked (`t/lift_check.py`'s own `L_inv_0` gap, LIFTER-785-RESIDUALS.md's new dated section has the full account). 27 of the 29 share one shape (a `for`-desugared range-bound local with no fact tying its value to anything, making the loop invariant's `<==>` obligation state a claim about an arbitrary `h` rather than the one the lift's own construction produces); the fix (`_task_var_inits`, a `requires h == <its own init expr>`) is sound, not a weakening: a genuinely non-equivalent task still refuses. Measured, same command as LIFTER-785.md's own run scoped to the 164 (`t/lifter.py --list <the 164> --jobs 4 --timeout 200`): baseline reproduces the census exactly (`lifted: 59`, `lift-check-failed: 29`); post-fix, `lifted: 72`, `lift-check-failed: 16`, every other refusal reason unchanged, `59 + 13 = 72` exactly (no regression). 14 of the 27 still fail on a separate, pre-existing local-alignment bug (named, not fixed, in the RESIDUALS page); 2 are a third, `while`-shaped gap, untouched. Graded the 13 newly-lifted tasks in all seven at flake 3: 0 read `all seven` in this run (mostly `dafny`/`framac` `verified` against the twin's own `unproved`, which does not count as agreement): lifting past `lift-check-failed` is eligibility for the sweep, not agreement, and these 13 were not previously eligible at all. `lift-check-failed` as a named refusal row goes from 29 to 16; the coverage-table re-sweep this item's own DONE WHEN needs is still open. The independent check of this row added one caveat, recorded here: the new fact is read from the lifted task's own `var` initializer, the same trust the checker already places in its nat_clause and call_guards facts; a wrong initializer from the rewrite stage would pass L_inv_0 and be caught only by the lowered task's own verdict column, which by decision 8 never folds into the lift-or-refuse gate.

2026-09-11 (wave G, the commit after 72e6b94, one builder per kernel on the rows of COVERAGE-lifted-785.md its column blocked plus the 15 tasks the two lifter rows lifted, each independently checked): the kernel side of 16.2 moved for the first time since the census. By column, cells moved to verified/refuted: verus 8 of its 11 older rows (a task-level requires and a read-only local's defining fact threaded into the loop helper; decreases rendered with a constant shift so the next call's measure stays non-negative at the guard's boundary; a rotate-by-n indexing lemma; the nonlinear bridge tested on the substituted goal) and 9 of the 15 new; spark 5 (261, 273, 282, 445, 616: an outer requires over untouched params and an untouched local's defining equation carried into the loop helper's Pre; the builder claimed 610 too, the check read its twin as timeout and the claim was corrected here); lean 8 (the frame fact, a prefix local never reassigned in the loop, threaded into every definedness theorem); fstar 9 (the same frame-variable definition, and a decreases off-by-one). Rocq's builder spent the wave on its three 13.4 cells and reported no 16.2 rows. The twin ladder gained two rungs below every existing one, wrong-constant and wrong-operator (SPEC.md, The twins, rungs 9 and 10), so the five no-twin rows (234, 242, 269, 626, 792) and medianOfThree now carry a grounded wrong twin refuted in dafny and verus, with the 34 committed twins byte-identical. Named, not fixed, and the largest thing this wave found: on isNonPrime, isPrime, sumOfCommonDivisors, anyValueExists, containsSequence, containsK and isSmaller the invariant-drop twin VERIFIES in dafny, and lean's builder saw the same twin's own spec theorem prove (its verifier reads malformed there rather than mint a false refuted), while the harness's preservation witness says the twin must be refuted. Two kernels agreeing against the harness points at the witness, not the kernels: the next wave reads whether the preservation state it picks respects the facts a loop cannot change (the requires on parameters the loop never assigns, the prefix locals), and until then those seven rows cannot reach all seven. Still open by kernel, each with the kernel's own message in its builder's note: verus isNonPrime and isPrime (a divisor-bound lemma plus an exit bound the task does not state); spark splitArray, splitAndAppend (a nested quantifier under a case split at the step budget), isArmstrong, isNonPrime, isPrime; lean the seq append, slice and update composition on 240, 262, 586, 591, 625, 106, nonlinear arithmetic on 86, two-loop bodies (610, an abstain by policy); fstar isNonPrime, isPrime, sumOfCommonDivisors (a co-divisor lemma), anyValueExists, isSublist, removeElement (shapes it abstains on), isArmstrong (nonlinear div and mod at the pinned rlimit). The all-seven count is measured by the sweep that follows this commit, not claimed here; the lifter side reads 75 of 164 with full checks (the 785 re-lift during this wave read 74, task 578's differential run timing out under load and re-lifted alone).

2026-09-11 (wave H, the commit after 116f076, seven builders each independently checked). Kernel cells moved to verified/refuted: rocq 10 (swap and centeredHexagonalNumber of the older rows, eight of the 15 new: the invariant-drop exit witness threaded, a certificate for a plain seq return, a nonlinear-multiplication prelude block gated by feature); spark splitArray (a ghost lemma subprogram gnatprove proves once, chaining the concat and slice postconditions at one index), and 8 of the 15 new tasks read verified/refuted in spark; lean centeredHexagonalNumber (a generic sign bridge, case split and Int.mul_nonneg) while its composed get-lemmas for append, slice and update land proved but do not yet close 240, 262, 586, 591, 625 or 106 (grind does not split the ite inside a cited fact, and the detector misses the declare-then-reassign shape of 262; the builder's report claimed the lemma reached 262's file, the check found it did not); fstar and verus read the real verified on isNonPrime, isPrime and sumOfCommonDivisors through a divisor-bound lemma each column proves (a divisor k of n with 2 <= k < n has k <= n div 2) and an exit bound the loop's own guard implies. Two findings, both measured, neither patched: first, isArmstrong's lift is not faithful to its Dafny source (spark's builder read the task's ensures against the .dfy), a t/lift_rewrite.py bug, so its cells are not kernel gaps until the lift is fixed. Second, the twin-preservation builder resolved wave G's finding into two causes. On isNonPrime, isPrime and sumOfCommonDivisors the invariant-drop witness sits on a loop counter below the floor its initializer and its only increment give it (i = 1 for a counter that starts at 2 and only grows): the state satisfies the kept invariants and the guard, so the twin rule's preservation check accepts it, but no execution reaches it, and dafny's own interval inference proves the dropped invariant without being told, so dafny verifies the twin honestly; verus, lean, rocq and fstar read the same twin unproved, not refuted. On anyValueExists, containsK, containsSequence and isSmaller the preservation check evaluates one iteration and tests the kept invariants afterwards even when that iteration returned from the loop, where only the ensures must hold, so the witness is spurious. Both say the same thing: an invariant-drop twin whose witness no execution reaches is not a wrong program, and the ladder should not stop at it. The repair (a preservation state that respects an early return, and a reachability check on the state, or a witness taken from an actual execution) changes which rung some committed tasks land on, so it moves the banked matrix; that is a decision for the morning, not taken tonight, and until then the seven rows cannot reach all seven. The all-seven count is the sweep's to say (running after this commit).

2026-09-12 01:25Z, sweep r20 (802c8b3, run alone at 32 jobs over the 302 run-ready lifted tasks after waves E to H; its commit message says E to G, it followed H): 48 of the 164 read verified/refuted in all seven kernels, from 34; the distance to 82 is 34. Of the 75 lifted, 27 are blocked; by column the blocking cells are framac 22, lean 17, spark 15, rocq 14, fstar 10, dafny 9, verus 9 (t/COVERAGE-mbpp-dfy-lifter.md names each row's blocking kernels and t/COVERAGE-lifted-785.md's blockers section the cells). Seven of the 27 are the preservation-witness rows above (a twin no kernel refutes), one is isArmstrong's unfaithful lift, and the rest are kernel capability by name: framac's seq returns and nested seqs, lean's seq composition, spark's nested quantifier and the primes, rocq's pair-of-seq and multi-loop abstains, fstar's abstained shapes. The lifter side stays at 75 of 164 (the 785 re-lift in this sweep read it, task 578 re-lifted alone after a differential-run timeout under load); the lifter rows the census names next are function-contract, real, set, heap, array. test_mbpp_lifter_census.py now pins 75 lifted, 75 sweep rows and 48 all seven, and skips while out/lift is newer than the sweep table instead of pinning a number nobody measured.

2026-09-12 (wave I, the commit after e4b16a0, eight builders each independently checked). Kernel cells moved to verified/refuted: framac 4 (swapFirstAndLast twice, removeElement, countLists: the certificate replay now enters while bodies, unrolled at the concrete witness, so the twin's refutation reaches WP as ground facts); lean isPrime (the divisor-bound lemma in lean core; its ite-free corollaries for composed seq lemmas land proved but do not yet close 106, 240, 262, 586, 591, 625, each with a diagnosed remaining gap in the file's note); rocq splitArray and tetrahedralNumber (the pair-of-seq component through the existing pair extension, and user identifiers that collide with the lowering's t_ namespace renamed by the names pass; the merge narrowed that rename to binders, since the builder's set had swept in the task's own name and renamed row_max_len for no collision, caught by test_names.py); fstar anyValueExists, isSublist, removeElement reals (a quantifier in computational position, a return in the loop prefix, two sequential loops chained), the last two with refuted twins; verus lucidNumbers, and anyValueExists's real (a trigger term found inside a nested quantifier). Spark re-measured its two FLAKED rows alone: both verified/refuted, so a 32-job sweep can read a spark twin as timeout that a quiet run refutes; the next sweep runs spark at lower parallelism or re-measures its timeouts alone. The lifter: lift_resolve.check_against_source re-parses every requires, ensures, invariant and decreases clause from the original .dfy text with the same grammar and, where the tree from dafny's print disagrees, takes the source's tree and records the disagreement in the sidecar, so lift_check's lemmas read the source side from the text (LIFTER-DECISIONS.md row 32); isArmstrong re-lifts faithfully, the 164 re-lift reads three methods newly clean and twenty sidecars carrying a recorded disagreement, no regression. Framac's own report: the divisor-bound lemma is written and unwired, because alt-ergo 2.4.3 cannot discharge the product fact q >= 2 and k >= 2 and n = q*k implies 2*k <= n at fifty times the pinned budget (probes in the builder's note), so isNonPrime and isPrime stay open in that column by the prover's own limit; 106, 460 and 86 need a frame fact asserted inside the loop body, the rendering site of the other framac builder, named for the next pass. Still open by kernel after this wave, by name: framac's six seq cells (nest_eq now reads an honest refusal instead of a crash), lean's seq composition six and the timeouts on 470, 578, 603, spark's splitAndAppend, the primes, sumOfCommonDivisors, elementAtIndexAfterRotation, lucidNumbers and removeElement's twin, rocq's removeElement (two loops) and splitAndAppend (rotate by mod). The all-seven count is the next sweep's to say.

2026-09-12 03:25Z, sweep r21 (the commit after 4435f73, alone at 16 jobs over the 302 run-ready tasks after wave I, isArmstrong re-lifted with the source cross-check): 53 of the 164 read verified/refuted in all seven kernels, from 48; the distance to 82 is 29. Of the 75 lifted, 22 are blocked; by column framac 17, lean 15, rocq 12, spark 11, dafny 8, verus 7, fstar 7. isArmstrong now reads verified/refuted in six columns and its rocq twin timed out in the sweep (marked FLAKED by blockers.py; a quiet re-measurement is below). Seven of the 22 are the preservation-witness rows, the rest kernel capability by name in the sweep table's blockers section. The census test pins 75 lifted, 75 sweep rows and 53 in all seven.

2026-09-12 (wave J, the commit after c9e0030, five builders each independently checked). Kernel cells moved to verified/refuted: framac appendArrayToSeq (a scalar prefix local never reassigned in the loop becomes an equality loop invariant, and a capacity-mode bare seq assignment copied against the target's capacity instead of the source's length, a real bug that made the task's own length invariant false at entry; getFirstElements stays on a doubly-indexed quantifier alt-ergo cannot instantiate at budget, centeredHexagonalNumber on a nonlinear ensures with no loop at all, pairwiseAddition is malformed at the Frama-C parser, a boolean subtraction in the div-mod bridging assert, diagnosed); lean swapFirstAndLast twice (a generated per-site script for the composed double-update shape, simp supplying the indices from the goal; the append-of-slices shape closed one conjunct in a probe and stopped on an ite inside a hypothesis); spark splitAndAppend (the rotate-by-n indexing law as a ghost lemma subprogram) and removeElement (the twin's certificate replayed into while bodies); dafny isSublist (a trigger annotation naming the slice subterm on two existential quantifiers, an e-matching hint, and the verifier's banned-attribute pattern carved out for trigger by exact name). Rocq's verifier reads coqc's No applicable tactic as unproved rather than malformed (sumOfCommonDivisors's real side), no lowering change. Spark's builder wrote and proved a divisor-bound lemma family standalone at the default step budget but did not wire it: the lemma needs the loop counter's post-loop value, which the loop helper's state threading does not surface at the return site; named for the next pass, the primes stay timeout in spark. Still open by kernel, by name: framac's abstains (240, 262, 414, 576, 577, 586, 603, 69) and the six 13.4 seq cells, unreached; lean's append-of-slices four, the three timeouts, two-loop bodies, isSublist; rocq's two-loop bodies, rotate by mod, pairwiseAddition's twin, isSublist, the primes' real side. The all-seven count is sweep r22's to say.

2026-09-12 05:20Z, sweep r22 (the commit after b9fc3b1, alone at 16 jobs over the 302 run-ready tasks after wave J, no flaked cell): 56 of the 164 read verified/refuted in all seven kernels, from 53; the distance to 82 is 26. Of the 75 lifted, 19 are blocked; by column framac 16, lean 13, rocq 11, spark 9, dafny 9, verus 7, fstar 7; seven of the 19 are the preservation-witness rows, the rest kernel capability named in the wave notes above. Tonight's arc: 34 after the census (2026-09-11), 48 after sweep r20, 53 after r21, 56 after r22, on a lifted set that went from 59 to 75. The census test pins 75 lifted, 75 sweep rows and 56 in all seven. One regression in that table, found reading it and fixed in the commit after 72eb2ab: dafny's cells for appendArrayToSeq and interleave read unproved because wave J's trigger annotation was stated on every quantifier, overriding dafny's own choice; it is now stated only on a slice application, the one shape dafny cannot auto-trigger, and the three rows (isSublist kept, the two regained) read verified/refuted in dafny alone at flake 3; the table's two cells are stale until the next sweep, and the all-seven count does not move since lean blocks both rows.

2026-09-12 08:37Z, sweep r23 (the commit after 4c1d2e6, alone at 16 jobs, the first sweep on the reordered twin ladder of d38da32): 45 of the 164 read verified/refuted in all seven kernels, from 56. The drop is the reorder's cost on this corpus, the same as on the matrix: dafny, verus and fstar now block no row at all (the seven preservation-witness rows are cleared in those columns), while lean blocks 25 rows and rocq 24, eighteen each with the twin reading unproved, framac 12, spark 5. Eighteen lean and eighteen rocq cells are one mechanism per kernel: a value-witness certificate replayed through a loop body, which framac and spark already have and lean and rocq do not (wave L's two items, paused tonight). Three twins crashed a lowering instead of being refused by name (verus: a loop body that assigns nothing after collapse-if; fstar: a KeyError on a variable the twin removed, twice), recorded as LOWER-ERROR by the sweep, each a named refusal to add. The count is lower and honest: a REFUTED twin on this corpus is now a program whose output the kernel confirmed differs. The census test pins 45.

2026-09-12 ("twin-order", the commit after b9fc3b1): the seven preservation-witness rows wave H and sweeps r21/r22 carried as an open finding (isNonPrime, isPrime, sumOfCommonDivisors, anyValueExists, containsSequence, containsK, isSmaller) are resolved, and the resolution is a rule change, not a per-task fix. An INVARIANT-DROP twin is not a wrong program: every reachable loop-head state still satisfies every SURVIVING invariant, so no reachable witness can refute it, and a kernel that verifies it by re-deriving the dropped annotation (dafny's interval inference on isNonPrime/isPrime/sumOfCommonDivisors; lean on containsSequence/containsK) is not unsound -- SPEC.md's own text already said a REFUTED invariant-drop twin means the kernel could not re-derive the invariant, which is the same claim read the other way. So `harness.twin_for`/`ladder_rungs` now try the EXTENSIONAL rungs first (their existing order unchanged) and invariant-drop last: a body with a behavioral twin (a value witness falsifying `ensures`) gets that twin, and invariant-drop remains the twin only where no behavioral rung exists. Separately, `interp.invariant_witness`'s preservation check had a real bug the reorder alone would not have caught on every task: it ran one loop iteration and demanded a surviving invariant hold at whatever state that iteration reached, including a state reached by a `return` inside the loop body -- a state the loop never revisits, where `ensures` is the obligation, not a survivor (SPEC.md "Early exit"). Fixed to check `ensures` at a returning iteration instead (the seven rows' own shapes: `anyValueExists`, `containsK`, `containsSequence`, `isSmaller` all return from inside their loop). `harness.decorative_kind` gained a third label, "re-derived", for a VERIFIED/VERIFIED pair whose witness is an invariant-drop proof witness (kind `exit`/`preservation`): a fact about that kernel's inference, never folded into "unsound" (reserved for a VALUE witness a kernel verified anyway). Measured: of the 34 committed tasks, 13 get a new twin (`all_nonneg`, `contains`, `count_matches`, `count_vowels`, `digit_sum`, `filter_pos`, `first_even`, `is_prime`, `linear_search`, `reverse`, `row_max_len`, `seq_max`, `sum_upto`, every one previously invariant-drop, now a behavioral rung already on the ladder); the other 21 keep the exact twin body, byte-identical. Graded in all seven kernels at flake 3 (`t/grade.py --tasks <the 13> --flake 3 --jobs 8`): every column that read `refuted` before still reads `refuted`; the only cells that moved were ones that read `verified/verified` ("unsound") or `verified/unproved`/`verified/timeout` before under the OLD twin and now read a plain, honest non-refuted outcome under the NEW one (spark and lean/rocq mostly: `all_nonneg`, `contains`, `count_matches`, `linear_search`, `row_max_len`, `seq_max` in spark; `is_prime`, `reverse` in lean and rocq; `first_even` in lean; `filter_pos` in spark and rocq; `digit_sum`, `sum_upto` unaffected in every column), each a kernel-capability gap on the NEW twin's own operator (collapse-if, compare-flip, boundary-swap), not a regression this bar tracks. The seven named rows (`t/out/lifted-tasks`, dafny_synthesis task_id 3, 605, 126, 414, 69, 808, 809) in dafny and verus at flake 3: before, 11 of 14 cells read `verified/verified` and 3 read `verified/unproved`; after, all 14 read `verified/refuted`. The regression bar itself (t/COVERAGE-lifted-785.md's dafny_synthesis rows that read `verified / refuted` today, dafny and verus columns, whose twin op changed under the reorder -- 16 distinct tasks, `appendArrayToSeq`, `elementWiseDivision`, `subtractSequences`, `elementWiseSubtraction`, `multiplyElements`, `getFirstElements`, `pairwiseAddition`, `interleave`, `arrayToSeq`, `lucidNumbers`, `elementWiseModulo`, `elementWiseDivide`, `findSmallest`, `addLists`, `squareElements`, `smallestListLength`): all 32 cells (16 x 2) still read `verified/refuted`, none moved. The conformance suite's manifest (66 items, dafny and verus columns only, 132 cells): zero diffs before vs. after, `fz_p_vac_post` unchanged (`verified/verified`, PASS, still `decorative`), no PASS lost. The merge gate banked the matrix on the new twins (t/AGREEMENT.md, 06:55Z): every real still verified, 20 of 34 rows in all seven, from 26, and every moved cell is a twin a kernel cannot yet refute because its certificate path does not enter a loop body for a value witness: spark reads unproved or timeout on the twin of all_nonneg, contains, count_matches, linear_search, row_max_len, seq_max, filter_pos and count_vowels; lean on first_even, is_prime, reverse; rocq on filter_pos, is_prime, reverse. Those rungs are collapse-if, compare-flip and boundary-swap, each with an ensures-falsifying value witness, so each of these cells is a certificate the kernel column owes, the same certificate through loops framac and spark gained on 2026-09-12 for undefined witnesses; digit_sum and sum_upto read verified/refuted in all seven on their new twins, and the 21 committed tasks whose twin did not change are unmoved. The count is lower and the claim is stronger: a REFUTED twin is now a program whose output differs from the real at a concrete input the kernel confirmed.

2026-09-14 (wave N, the commit after 010170f: four builders in separate worktrees, each independently checked; HANDOFF-2026-09-12.md items 1 and 2). Value-witness certificates through loop bodies landed in lean and rocq, the mechanism framac and spark have had since 2026-09-12. lean (t/lower_lean.py): a collapse-if twin's guard renders as the literal True, which left a dead recursive branch whose decreasing_by obligation Lean cannot close; the resulting sorryAx made the loop function uncomputable for every certificate tactic. The loop now omits the recursive call when the guard-true step always returns, and a new _cert_undefined_loop replays an undefined witness through the while by concrete unrolling with interp. Committed rows first_even, is_prime, reverse: verified/unproved to verified/refuted, dafny unchanged. Of eight lifted rows probed, six moved to refuted (arrayProduct, arraySum, double_array_elements, linearSearch, rotate, mmaximum1) and two did not (Clover_cal_sum.Sum off-by-one and downWhileGreater compare-flip; lean's own message "unsolved goals, case refine_2, False" through the domain-hypothesis loop shape, named open). The 34-task matrix in lean matches AGREEMENT.md except the three intended moves; lean conformance 66 of 66 unchanged; the 63 dafny_synthesis rows reading verified/refuted in lean all still do, and three more twins (3 isNonPrime, 106 appendArrayToSeq, 414 anyValueExists) moved unproved to refuted with the real untouched. rocq (t/lower_rocq.py): three narrow gaps downstream of an already-correct Fixpoint value. _undef_cert refused any task with a loop; a new _undef_walk unrolls the while at the witness. The seq-return branch of _value_cert now grounds the twin's output length by vm_compute before the repeat match and closes with lia, congruence, or intuition congruence. New _forall_true_quants and _forall_proof_block prove a bounded forall true at the witness by enumeration. Committed rows filter_pos, is_prime, reverse: verified/unproved to verified/refuted, dafny unchanged. 17 of the 75 dafny_synthesis rows moved to refuted on the twin side, 58 unchanged; of the 30 named lifted rows 29 read verified/refuted, and 603 lucidNumbers still reads unproved ("No applicable tactic": the seq branch specializes every forall at one shared index, while the witness falsifies the first conjunct at index 1; named open). The 34-task matrix in rocq matches except the three moves (min_max stays timeout/refuted, its source byte-identical); rocq conformance 66 of 66 unchanged. verus (t/lower_verus.py): the collapse-if twin of mfs_tmp_tmpmmnu354t_testes_anteriores_t2_ex5_2020_2__leq that tripped "loop body assigns nothing in scope" is a loop whose body always returns; a new _always_returns lowers that shape (the recursive tail call is dead), and every other empty-state shape raises NotImplementedError("verus: collapse-if twin: loop body assigns no variable in scope"), a named abstain instead of a crash. The row reads unproved/refuted (the real's unproved is a pre-existing verus gap on that body, named). Matrix and verus conformance unchanged; the 75 dafny_synthesis rows lower byte-identical. fstar (t/lower_fstar.py): the two KeyErrors (flexweek_tmp_tmpc_tfdj_3_ex3__max on i, dafny_workout_tmp_tmp0abkw6f8_starter_ex09__computeFib on c) came from gen_loop counting a local that exec_flow never reached once collapse-if made the early return unconditional. When every path through the prefix returns, the function now lowers to that value under the task's own signature; any other missing binding raises NotImplementedError naming the variable. max reads verified/refuted; computeFib reads unproved/refuted (the real's unproved is F*'s own gap on the recursive fib body, previously hidden inside the lower-error bucket). Matrix and fstar conformance unchanged; the 75 rows lower byte-identical. Each checker hand-tampered one certificate in its kernel (lean's negation stripped, rocq's Ht_rlen and t_out, verus's claimed value, fstar's encoded return) and each kernel rejected it. Gate (tup-reports/vram/wave-gate.sh wave-n): conformance 454 of 462 with the same eight framac cells, and five probe rows (fz_p_ret_first, fz_p_ret_falsens, fz_p_upd_first, fz_p_seqeq_true, fz_p_seqeq_false) whose twins moved from unproved to refuted in lean and rocq, PASS before and after; the 34-task matrix moved exactly filter_pos, first_even, is_prime and reverse from unproved to refuted on the twin side in lean and rocq, no real moved, no other cell moved, AGREEMENT.md re-banked, and lean and rocq now block no matrix row alone (of the eight rows in six, seven are spark alone and one framac alone); reproduce.sh --tests: every test passed except test_lsp.py, which fails identically at d38da32: t/lsp-transcript.jsonl (wave C, 2026-09-11) recorded file URIs under a worktree deleted since, and test_diagnostics_match_check_file opened them as written; the test now rebases each recorded path onto its own directory by the t/-relative tail, fixed in this commit. New tests, wired into reproduce.sh: t/test_lower_lean_loop_cert.py, t/test_lower_rocq_loop_cert.py, t/test_lower_verus_refusals.py, t/test_lower_fstar_refusals.py. No sweep this wave; the census test still pins 45, and sweep r24 is the next measurement.

2026-09-14 22:49Z, sweep r24 (the commit after 2e0f9c0, alone at 16 jobs over the 302 run-ready lifted tasks after wave N): 54 of the 164 read verified/refuted in all seven kernels, from 45. Cells moved, 55 rows of 302 touched, none on the real side except the two former crash rows: rocq 35 twins from unproved to refuted with the real verified, 7 more with the real unproved and 3 with the real timeout; lean 31 twins from unproved to refuted with the real verified, 9 with the real unproved and 1 with the real timeout; fstar's two former lower-error rows read verified/refuted (flexweek ex3 max) and unproved/refuted (dafny_workout ex09 computeFib); verus's former lower-error row reads unproved/refuted. Sole blockers after r24: spark 19 rows, lean 7, rocq 6, verus 5, framac 5, dafny 0, fstar 0 (r23: spark 10, lean 5, rocq 4). One cell moved the wrong way and is fixed in this commit: final_project_dafny_tmp_tmpmcywuqox_attempts_exercise3_increment_array__incrementArray (a seq parameter, an update inside the loop body, a compare-flip twin whose witness is undefined one iteration past the end) read lower-error on both real and twin in lean, "TypeError: can only concatenate list (not tuple) to list", where r23 read verified/unproved: _cert_undefined_loop handed the witness's seq values to interp.exec_body as the plain lists the witness renders them as, and interp.py's update builds tuples. The replay env is now tuple-ized before the loop runs (t/lower_lean.py, a dated note; t/test_lower_lean_loop_cert.py pins the shape inline), and the row graded alone under the fix reads verified/refuted in lean and dafny, so with its other five cells already verified/refuted it reads all seven. The table and t/test_mbpp_lifter_census.py pin the count as swept, 54 (lifted 75, sweep rows 75, all seven 54, all 54 lexically in fragment); sweep r25 re-pins. The lifter-side greedy now starts from 54: lift-check-failed still gates the most rows, then unbounded-quantifier, array, function-contract, wave O's four lifter items.

2026-09-15 (wave O, the commit after 5a6a73e: nine builders in separate worktrees, each independently checked; HANDOFF-2026-09-12.md items 3 and 4, the spark matrix blocker, and wave N's named residuals). The lifter side, the four items the greedy order names. check-scopes (t/lift_check.py, t/lift_rewrite.py): lift_rewrite now records the for-desugared bound's own name at rewrite time and lift_check reads the flat union of those names instead of reconstructing the extra local from declaration order, and a new array_view_fact (forall k, a[k] == a[..][k], one conjunct per array-typed param or return) is threaded into L_ens and L_inv_k; of the 16 named programs 6 lift cleanly (267, 307, 743, 770 by the alignment; 433, 284 by the array view), 555 is blocked by a genuinely false biconditional (the source's literal invariant is weaker than the for-loop's own range fact), 565 and 759 are open by name. check-seqlocals (t/lift_check.py, the lemma builders): _find_fun_calls now returns the innermost quantifier whose binder a hinted call mentions, the hint is re-quantified under a matching forall statement instead of pasted bare, and _array_index_guards supplies the array bound for an indexed argument; 436 and 804 lift with every lemma verified, 412, 426, 554, 629 stay on lower_dafny's own loop-invariant-maintenance gap on the lowered method (reproduced outside any checker file), 732 on the type-swap alignment, 594 on a genuine open obligation. lifter-eqfilter (t/lift_classify.py, LIFTER-DECISIONS.md row 33): a binder pinned by an equality conjunct in the quantifier's filter is eliminated by substitution before the bounding rules run, both operand orders, several binders, a self-reference guard; five unit tests; no corpus row moves, because row 32's source-text read already bypasses every in-corpus occurrence of the resolver's _t#0 artifact, so the row is defense in depth and says so. lifter-contracts (rows 34 and 35): a read-only array parameter lifts as a seq (two false refusals fixed, plus an UnboundLocalError on an alloc-fill local and a SetDisplay decreases the rewrite could not map), and a closure function whose reads clause names only its own read-only array parameter is accepted; 6 of the 8 named programs pass the classifier (447, 755 arrays; 2, 161, 249 function contracts), 94 and 143 stay refused (array<seq<int>> and seq<array<int>>, not the decided shape); 2, 161, 249 and 447 produce tasks dafny verifies but fail the lifter's own spec-function equivalence lemma for an array-to-seq parameter, a lift_check gap named for the next checker item. Lifter regression bar, the 164 re-lifted on the merged tree (t/lifter.py --list, jobs 4, timeout 200, while the gate ran) and diffed against t/out/lift: 94 of the 164 lift, from 75; 20 newly lift (70, 145, 267, 284, 307, 412, 426, 433, 436, 472, 554, 567, 594, 622, 629, 743, 760, 770, 775, 804), no previously lifted task changed, and one, 578 interleave, read lift-diff-failed on the 200 second differential timeout under load and lifts byte-identically alone in 196 seconds; 14 still-refused programs changed the token they refuse on (2, 161, 249 and 447 now reach the checker and fail its spec-function lemma for an array-to-seq parameter; 755 reaches check_wf and is refused for a spec_fun whose decreases is not int; 579 reads ghost-local). The fresh lift is installed as t/out/lift for those programs and t/out/lifted-tasks is rebuilt by select_lifted.py, 322 run-ready tasks from 302. The kernel side. spark (t/lower_spark.py): value-witness certificates through loop bodies, the mechanism the other six kernels had; every loop helper gets an uncontracted clone W_k_Cert and the certificate replays the twin at the witness through it. All seven sole-blocked matrix twins moved from unproved or timeout to refuted (all_nonneg, contains, count_matches, filter_pos, linear_search, row_max_len, seq_max), min_max and count_vowels too, every real unchanged; every twin cell in spark's 34-task column now reads refuted; six of the ten spark-sole lifted rows probed all moved (minArray twice, maxArray, mfirstMaximum, findSmallest, smallestListLength), the other four are the sweep's to say. framac (t/lower_framac.py, DESIGN-framac-nested-seq.md sections 1 to 4): fz_p_nest_empty, fz_p_str_splitempty and fz_p_str_lowernonletter read verified, each as a compile-time fold of the design's construction rather than the general scanning loop, which the note says; fz_p_str_tab's local now declares but the cell abstains for a deeper reason (seq equality in executable position); fz_p_pair_seq and 262 splitArray not attempted (section 5); fz_p_nest_eq and 69 containsSequence still time out on the same alt-ergo instantiation goal wave K named. lean, composed goals (DESIGN-lean-seq-composition.md): 240 replaceLastElement reads verified/refuted and 262 splitArray held across four re-grades; 106 appendArrayToSeq stays on the loop-preservation leaf (a bare grind there was measured flaky and reverted), 586 on a modulo-indexed rotation, 470, 578, 603 on grind heartbeats, 576 on an unrelated elaboration error, 610 on two sequential loops. lean, loop certificates (the two rows wave N named): _cert_value_loop replays the raw loop at a value witness whenever the loop needs the domain hypothesis, never trusting the compiled function whose definition-time obligation the twin falsifies; Clover_cal_sum.Sum and downWhileGreater read refuted, and of 27 lifted rows with that shape 15 moved and 12 stay open by name (computePower's dropped-bound class among them). rocq: 603 lucidNumbers reads verified/refuted through a per-conjunct falsifying index for every forall node, a match arm per conjunct, and the seq equality shape that renders as a forall without a node (an intermediate version cost one conformance cell and was caught); of the 89 other lifted rows whose rocq twin read unproved or timeout at r24, 33 moved to refuted graded alone and five stay on two different gaps named with coqc's messages. Every checker hand-tampered one certificate or one lifted task and the kernel or the checker rejected it. Gate (tup-reports/vram/wave-gate.sh wave-o): conformance 457 of 462, from 454, the five FAIL cells being biglen and seqlen (open by design), pair_seq, nest_eq and str_tab (named above); the 34-task matrix moved exactly spark's nine twin cells to refuted (all_nonneg, contains, count_matches, count_vowels, filter_pos, linear_search, min_max, row_max_len, seq_max), no real moved, AGREEMENT.md re-banked, and of the two rows still short of all seven one is framac alone (count_vowels, abstain) and one rocq alone (min_max, the flaky real timeout); reproduce.sh --tests all pass, round trip on 1782 tasks. New test: t/test_lower_spark_loop_cert.py, wired into reproduce.sh; the other test files grew in place. The all-seven count is sweep r25's to say.

2026-09-15 01:47Z, sweep r25 (the commit after 431eda3, alone at 16 jobs over the 322 run-ready lifted tasks after wave O; a first attempt at 00:44Z was OOM-killed at 457 GB in lean's lowering of clover_linear_search1's twin, fixed in 431eda3, and a lowering probe over all 322 tasks read clean before this run): 56 of the 164 read verified/refuted in all seven kernels, from 54; the lifter side reads 95 of the 164 lifted, from 75. Cells moved on the 302 old rows, 42 rows touched, no real left verified: spark 17 twins from unproved to refuted and 12 from timeout to refuted with the real verified, five more with the real unproved or timeout; lean 15 twins from unproved to refuted with the real verified, three more with the real unproved or timeout, incrementArray from lower-error to verified/refuted, one row from unproved/refuted to verified/refuted; framac maximum (dafny_language_server tutorial) from abstain to timeout/refuted. One cell moved the wrong way: pancakesort flip's spark twin from refuted to timeout, at the 180 second wall in the sweep's contended tail, named for the next spark item. Of the 20 newly lifted rows one reads all seven (145 maxDifference), and the other 19 expose kernel gaps on shapes the lifter never reached before: the five closure-predicate programs (412, 426, 436, 554, 629, a spec_fun called in executable position) read dafny unproved on the real, framac abstain, lean and fstar timeout; 433 and 567 read dafny vacuous on the real; 472, 567, 622 read verus malformed; 307 and 743 read framac and lean abstain; 284 and 760 are blocked in most columns. Sole blockers after r25: spark 9 rows, rocq 8, lean 7, verus 6, framac 6, dafny 1 (433), fstar 0. The census test pins lifted 95, sweep rows 95, all seven 56, all 56 lexically in fragment. The lifter-side greedy now starts from 56; what the newly lifted rows need is on the kernel side, the closure-predicate shape first (five rows, every kernel).

2026-09-15 (wave P, the commit after f6a964a: eight sonnet builders in worktrees, each independently checked; the closure-predicate shape sweep r25 named in five kernels, verus's malformed rows, the checker's spec-function lemma, the spark and rocq sole blockers). dafny (t/lower_dafny.py): the five closure-predicate reals (412 removeOddNumbers, 426 filterOddNumbers, 436 findNegativeNumbers, 554 findOddNumbers, 629 findEvenNumbers) read verified/refuted at flake 3, from unproved on the real; the cause was the lowering's own loop-invariant maintenance, "this invariant could not be proved to be maintained by the loop" at the invariant carrying the lifter's membership rewrite (exists k :: lo <= k < hi && seq[k] == elem, an index witness whose trigger dies when the sequence grows); expr's exists branch now recognizes exactly that shape through a new _seq_membership and lowers it as Dafny's native elem in seq, the operator the untouched source verifies with. 433 isGreater and 567 isSorted still read vacuous on the real: dafny's own --warn-contradictory-assumptions diagnostic, "proved using contradictory assumptions: index in range", fires identically on DafnyBench's untouched hand-written source for both, so it is the source's own shape and not the lowering's, named open. The 34-task dafny column, dafny conformance (66 of 66) and the 88 dafny_synthesis rows reading verified/refuted are unchanged. verus (t/lower_verus.py): 472 containsConsecutiveNumbers, 567 isSorted and 622 findMedian, malformed at r25 ("Could not automatically infer triggers for this quantifier", a body reading one seq at the bound variable and again at a constant offset), read verified/refuted through two additions, _offset_at_by_var and _root_plus_offset_trigger, which emit the root and the offset read as independent trigger groups; the five closure-predicate reals stay unproved, and the cause measured is not the spec fn in executable position (legal verus) but "precondition not satisfied" at the loop helper's own recursive tail call, where the existential membership invariant does not re-derive across the seq append that grows the output; a prefix-preservation assert alone was measured insufficient, dafny's real read unproved on the same shape before its fix, named open. framac (t/lower_framac.py): a non-recursive spec_fun over int parameters called in executable position is mirrored as a C function named for it with an ACSL contract stating agreement with the logic function (ensures (\result != 0) <==> f(args) for a predicate), and cexpr's call case dispatches to the mirror instead of abstaining; the mirror's own contract proves (12 of 13 goals on the standalone probe), and beneath it the same five rows reach a second, pre-existing abstain, "seq return length is not statically determinable ... no ensures gives a CAPACITY bound" (a filtered variable-length output whose bound lives in the loop's own invariant, not the ensures), named for the next framac pass; 307 deepCopySeq and 743 rotateRight abstain on a plain seq-typed local that DESIGN-framac-nested-seq.md's seq-of-seq encoding does not reach, named. An ineligible call (seq-typed parameter, or self-recursive as in factorialOfLastDigit) still abstains with the original message. lean (t/lower_lean.py, the certificate functions untouched): a spec_fun called under a quantified invariant made grind ematch over the function's equation and the invariant list together; sfun_quantified and _grind_base now cite the equation at the loop's instance, and _loop_zero gained a seq case. 436 moved timeout/timeout to timeout/refuted, 307 abstain/abstain to verified/unproved, 743 abstain/abstain to unproved/unproved (its loop spec still carries a sorryAx, a loop-lowering gap named open), 412, 426, 554 and 629 stay timeout/timeout on the preservation goal's own instantiation over two sequences ("(deterministic) timeout at 'whnf', maximum number of heartbeats"), and 775 isOddAtIndexOdd holds at verified/timeout after a first version regressed its real and was caught by re-grading. None of the 34 committed tasks uses a spec_fun, and their emitted lean is byte-identical. fstar (t/lower_fstar.py): no code moved; the five timeouts are a Z3 matching loop on the forall-exists membership invariant, independent of the spec_fun call (an unfold, inlining the predicate away, and an explicit exists pattern each measured not to close it), and 284 allElementsEqual and 760 hasOnlyOneDistinctElement's unproved reals are F* Error 19 at an early-exit branch that needs the local value as an explicit existential witness; a dated docstring note names both. The checker (t/lift_check.py, LIFTER-785-RESIDUALS.md): _fn_param_views had no array case, so a closure spec_fun's array-typed argument was passed raw where its lifted parameter is a seq, and the method-level views never named an alloc-fill array local in loop scope (447's cubedArray); a shared _view_kind_for_type, the array case, and a loop_views merge feeding L_inv_k fix both, and 2 SharedElements, 161 RemoveElements, 249 Intersection and 447 CubeElements lift with every lemma verified by dafny. Lifter regression bar, the 164 re-lifted on the merged tree (t/lifter.py --list, jobs 4, timeout 200, while the gate ran) and diffed against t/out/lift: 99 of the 164 lift, from 95, the four newly lifted being exactly the checker item's four (2, 161, 249, 447); no previously lifted task changed; seven programs read a 200 second timeout under the gate's load (145 maxDifference at dafny resolve, 2, 161, 106, 234, 238 and 578 in the differential) and were re-lifted alone at two jobs, where every one but 578 Interleave lifts as before, which timed out again alone at one job (its differential needs 196 seconds on an idle box, measured at wave O, and the rocq builder was grading on this one); its task JSON is byte-identical to the reference, so the wave O entry is kept; five still-refused programs moved their refusal token from L_req or the closure lemma to L_ens (18 RemoveChars, 230 ReplaceBlanksWithChar, 474 ReplaceChars, 741 AllCharactersSame, 732 ReplaceWithColon), the loop-scope views reaching a lemma they did not reach before, still refused. The fresh lift is installed as t/out/lift for those programs and t/out/lifted-tasks is rebuilt by select_lifted.py, 326 run-ready tasks from 322; the lowering probe over every task (twin_for and the fourteen lowerings, no kernel, under a 40 GB cap) read 326 of 326 lines, no crash, peak RSS 0.05 GB, no cell over one second. spark (t/lower_spark.py): _undef_obligation now surfaces an if or while condition as the undefined obligation (it returned None whenever the twin's first undefined operation sat in a condition rather than an assignment, so no certificate was emitted at all), and a derived invariant 2*V == N beside the running V == div(N, 2) lets gnatprove close the triangular-sum identity without a T_Div case split in the same goal; all nine spark-sole rows (mmaximum1, three findMax variants, lookForMin, findMin, find_min_index, gauss, triangleNumber) read verified/refuted standalone at DEFAULT_STEPS, the 34-task spark column and its conformance column unchanged; pancakesort flip's twin still times out standalone ("VC_PRECONDITION ... severity medium/limit; VC_SUBPROGRAM_VARIANT ... medium/limit", and ESCALATED_STEPS reaches the 180 second wall), named open. rocq's item, the eight sole-blocked rows and the two gaps wave O named, was still building when this commit was cut and lands in the next one. Every builder's claims reproduced under its checker, and the lean checker hand-edited a certificate's claimed value and saw lean reject it. Gate (tup-reports/vram/wave-gate.sh wave-p): conformance 457 of 462, the same five framac cells as wave O (biglen and seqlen timeout, pair_seq and str_tab abstain, nest_eq timeout); the 34-task matrix diffed zero lines against the banked AGREEMENT.md, no cell moved in any kernel; reproduce.sh --tests every file passed, round trip on 1782 tasks. New tests, wired into reproduce.sh: t/test_lower_dafny_closure.py, t/test_framac_spec_fun_exec.py, t/test_lower_lean_closure_predicate.py; t/test_lower_spark_loop_cert.py and t/test_lift_check.py grew in place. The all-seven count is sweep r26's to say.

2026-09-15 (wave P's eighth item, rocq, the commit after e1da5c9; its builder started late because the session's working directory had drifted outside the repository when the wave launched and no worktree could be created for it, so it ran on the resumed workflow while the other seven were gated). rocq (t/lower_rocq.py): the eight rocq-sole rows of sweep r25 reproduced alone at flake 3 with coqc's own messages, and two generic additions landed: a reflexivity arm tried first in t_leaf (a goal already definitionally equal on both sides closes without t_eqs's rewrite) and a split arm for a conjunctive goal in t_go and t_go_ext (neither had any rule for a conjunction, confirmed missing on a synthetic probe). testDouble reads verified/refuted in rocq and dafny, from unproved; isArmstrong reads verified/refuted alone, its r25 cell a contention timeout and not a gap. Six stay open by name: containsK (the file's own gated h = s_len invariant trade-off of 2026-09-11, a 60x wall-time cost when re-enabled for the return-bearing family), is_even (the reverted ground-substitution attempt of 2026-09-10, which turned unproved into timeout), and invertArray, extra_sum's sum and flex_ex2's max, together with wave O's search, max, computeFib, problem5 and sumOfCommonDivisors, on one deeper gap: the generic engine unfolds a recursive spec_fun's equation once per t_dis call and derives no bound or nonnegativity lemma for it (sf_fib(i + 1) >= 0, say), where Dafny unfolds without bound; a per-spec_fun bound lemma proved once by induction beside sf_<f>_eq is the named fix, to be load-tested before landing as the file's POST_SF_NIA note demands. mystery1 is a third proof script (gen_rec, a self-recursive method) that calls neither t_dis nor t_go, reproduced and not investigated. The 34-task rocq column is byte-identical to AGREEMENT.md (min_max's flaky timeout included), rocq conformance 66 of 66, the 71 dafny_synthesis rows reading verified/refuted in rocq unchanged; the checker hand-edited testDouble's twin certificate (its value 3 for 2) and coqc rejected it, "Unable to unify". On the merged tree: test_names.py, test_lower_rocq.py and test_lower_rocq_loop_cert.py pass, 45 of 45; grade.py over the 34 committed tasks in rocq and dafny at flake 3 matches AGREEMENT.md cell for cell, min_max's rocq real the recorded flaky timeout and every dafny cell verified/refuted; the lowering probe over the 326 run-ready tasks read 326 of 326 lines, no crash, peak RSS 0.05 GB, no rocq cell over 1.1 seconds. t/test_lower_rocq_loop_cert.py grew by five cases.

2026-09-15 17:07Z, sweep r26 (the commit after 1ce6a05, alone at 16 jobs over the 326 run-ready lifted tasks after wave P; the lowering probe over all 326 read clean before the run, and again after the rocq item): 57 of the 164 read verified/refuted in all seven kernels, from 56; the lifter side reads 99 of the 164 lifted, from 95; over the whole lifted corpus 182 of 326 read all seven, from 171 of 322. Cells moved on 31 of the 322 old rows, no real left verified, and eleven rows crossed into all seven: spark's nine sole-blocked rows (mmaximum1, the three findMax variants, lookForMin, findMin, find_min_index, gauss, triangleNumber), testDouble through rocq, and 622 findMedian, the one MBPP-DFY row, through verus. By kernel: spark, ten twins from unproved to refuted with the real verified, gauss and triangleNumber from timeout to verified/refuted, three more twins to refuted with the real still unproved; dafny, the five closure-predicate reals (412, 426, 436, 554, 629) from unproved to verified with the twin refuted; verus, 472, 567 and 622 from malformed to verified/refuted (567 stays out on dafny's vacuous real, 472 on rocq); framac, the abstains on 594, 775, 804 and ghost triple now read timeout on the real, 775 and 804 with the twin refuted, the CAPACITY bound wave P named beneath them; lean, 307 from abstain to verified/unproved and 743 to unproved/unproved; rocq, testDouble and allow42 from unproved to verified on the real. One cell moved the wrong way: 594 firstEvenOddDifference's spark twin from refuted to timeout in the sweep's contended tail, as pancakesort flip's did at r25 (that one held at timeout), named for the next spark pass. The four newly lifted rows reach the kernels for the first time: 2 sharedElements, 161 removeElements and 249 intersection read verus verified/unproved, framac, lean and rocq abstain and fstar malformed; 447 cubeElements reads framac abstain and lean verified/unproved, all seven elsewhere. Sole blockers after r26: lean 9 rows, rocq 8, framac 6, verus 5, dafny 1 (433), spark 1 (allow42), fstar 0, of the 30 rows in six (r25: spark 9, rocq 8, lean 7, verus 6, framac 6, dafny 1). The census test pins lifted 99, sweep rows 99, all seven 57, all 57 lexically in fragment. The greedy now starts from 57: lean's nine and rocq's eight rows first, then the closure-predicate family in verus, lean and fstar.

2026-09-15 (wave Q, launched at 05c5b8d: eight sonnet builders in worktrees, each independently checked; the sole-blocked rows of lean, rocq, verus, framac and spark, and the closure-predicate family in verus, lean and fstar). A staging slip first, named here: git apply --3way stages every hunk it applies, and the spec-experiment commit ceadb2a, made from that index, carried six of this wave's kernel patches (framac, fstar, lean, rocq, verus and their tests) before their gate ran; the gate below ran on that tree plus spark, and commits are by pathspec from now on. lean (t/lower_lean.py, lean-sole): two of the nine lean-sole rows read verified/refuted, pow and factorial, through _param_state_bridge_lines, nonnegativity facts for every product of an int parameter and an int loop-state variable in both orders, tried as one more alternative on the recursive-apply closer; seven stay open by name: mfirstCero and find need the frozen _cert_undefined_loop to treat a guard-level undefined witness as a certificate rather than a false guard (its own `if guard_now is not True: return None`), 106 appendArrayToSeq is the invariant-application leaf, 578 interleave and both getEven rows read timeout/timeout on a machine at load average 150 to 255 throughout the wave and were not separable from contention, arrayUpToN was not reached. The lean-closure item produced a patch but no report and no check reached the journal, so it is held back for wave R rather than merged unchecked. rocq (t/lower_rocq.py): _emit_t_eqs_rec, a per-recursive-int-spec_fun tactic that unfolds the function's equation inside a hypothesis at the right argument, resolves the exposed conditional and transports the result into the goal, with multimatch backtracking so a wrong occurrence fails and is retried, never accepted; extra_sum's sum and 598 isArmstrong read verified/refuted; six rocq-sole rows stay open, each a different mechanism (472, 808, invertArray and flex_ex2's max have no spec_fun and need a raw scan-loop invariant generator; is_even's bool spec_fun keeps its named case-split gap; mystery1's self-recursive method never reaches t_dis). verus (t/lower_verus.py, two items): the closure family 412, 426, 436, 554 and 629 and the three seq-of-array rows 2, 161 and 249 all read verified/refuted through an existential-carry proof at the append (verus_closure_ helpers, t/test_lower_verus_closure.py), and the five verus-sole rows (mroot1, the three mult, sum) read verified/refuted through nonlinear-arithmetic asserts with the loop's own facts as hypotheses (verus_sole_ helpers); the 34-task verus column and its conformance column unchanged. fstar (t/lower_fstar.py): 2, 161, 249 (malformed at r26), 284 and 760 (Error 19 at an early exit) read verified/refuted; the five closure timeouts stay open, a fourth lever (explicit forall_intro maintenance helpers for both membership invariants) measured worse, 150 seconds with no output, and was not kept. framac (t/lower_framac.py, t/test_framac_capacity.py): _ret_capacity now derives the output bound from the loop's own invariants and a local built by the append idiom folds into the return buffer, so the five closure rows and 307 and 743 lower and read timeout/refuted instead of abstaining; of the six framac-sole rows, 460, 86 and ghost triple lower and read genuine WP timeouts (goals named, 86's twin refuted), 240 and 262 abstain on larger shapes (a non-append concatenation into an exact buffer, a pair with a seq component), 577 on its self-recursive spec_fun; the checker's one objection was that 86's twin reads refuted where the builder wrote timeout. spark (t/lower_spark.py): compile_r's if-merge no longer joins a return variable seeded None across a branch that always escapes, so allow42 lowers to valid Ada and reads verified/refuted (spark's one sole row); 760's abstain becomes a genuine timeout and, with 284, needs the early-exit existential witness fstar and verus now carry; pancakesort flip's twin still times out standalone, its flipped-guard contract not self-maintaining. Every checker reproduced its builder's claims; the lean checker hand-corrupted factorial's certificate and lean read unproved with the audit message. Gate (tup-reports/vram/wave-gate.sh wave-q, after a first run under a stray 40 GB virtual-memory cap read 63 dafny tool errors and was discarded): the lowering probe over the 326 tasks clean; conformance 457 of 462, the same five framac cells; the 34-task matrix diffed zero lines; reproduce.sh --tests every file passed, round trip on 1782 tasks. New tests wired: t/test_framac_capacity.py, t/test_lower_verus_closure.py; the others grew in place. The all-seven count is sweep r27's to say.

2026-09-16 01:41Z, sweep r27 (the commit after e68c532, alone at 16 jobs over the 326 run-ready lifted tasks after wave Q; the lowering probe over all 326 read clean before the run): 57 of the 164 read verified/refuted in all seven kernels, unchanged from r26; over the whole lifted corpus 192 of 326 read all seven, from 182, ten rows crossing and none lost (mroot1, the three mult rows, mfes sum, extra_sum, pow, factorial, fatorial, allow42). Cells moved on 40 rows: verus 15 reals from unproved to verified with the twin refuted and 3 twins to refuted, the closure family (412, 426, 436, 554, 629) and the seq-of-array rows (2, 161, 249) among them; lean 8 reals from unproved to verified; framac 7 abstains to timeout/refuted (the CAPACITY rows and 307, 743); spark 3 abstains to verified/refuted and 4 to timeout/refuted, 1 twin from timeout to refuted; rocq 3 reals to verified (extra_sum, 598 isArmstrong, extra_mod); fstar 3 malformed rows to verified/refuted (2, 161, 249) and 2 reals to verified (284, 760). Three lean twins fell from refuted to timeout (two rows) or unproved to timeout (one) in the sweep's tail, named for the next lean pass. The 164 did not move because every closure and seq-of-array row is still held by lean, rocq or framac, the three columns wave Q could not close for them. Of the 30 tasks in six, 9 are rocq alone, 8 are lean alone, 6 are spark alone, 6 are framac alone, 1 is dafny alone. The census test's pins are unchanged (lifted 99, sweep rows 99, all seven 57, all in fragment). The greedy now starts from 57 again: lean's closure rows and the seq-of-array abstains in lean, rocq and framac are the rows that turn wave Q's kernel gains into a count.
